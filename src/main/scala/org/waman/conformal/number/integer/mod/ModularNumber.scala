...
abstract class ModularNumber
  ...
  override def toString: String = s"${valueAs[BigInt]} (mod ${modulusAs[BigInt]})"
}
...
object ModularNumber{
  ...
  private[mod] class ModularInt(val value: Int, val modulus: Int)
      extends ModularNumberAdapter[Int] with IntScalaIntegralNumber{ lhs =>

    override def valueAs[I: Integral] = implicitly[Integral[I]].fromInt(value)
    override def modulusAs[I: Integral] = implicitly[Integral[I]].fromInt(modulus)

    private def create(n: Int) = new ModularInt(n, modulus)

    override protected def calculateNegate = create(modulus - value)
  ...
  private[mod] class ModularLong(val value: Long, val modulus: Long)
    extends ModularNumberAdapter[Long] with LongScalaIntegralNumber{ lhs =>
    ...
    override protected def calculateProduct(rhs: ModularNumber) = {
      val prod = (lhs.toBigInt * rhs.toBigInt) % modulus.toBigInt
      create(prod.toLong)
    }
  ...
  private[mod] class ModularBigInt(val value: BigInt, val modulus: BigInt)
    extends ModularNumberAdapter[BigInt] with BigIntScalaIntegralNumber{ lhs =>
    ...
    override protected def calculateSum(rhs: ModularNumber) =
      create((lhs.toBigInt + rhs.toBigInt) mod modulus)
